...
df_native_all <- read_csv(paste0(root,"data/base/raw/previous/ALL_native-mammals_2019-09-01.csv"),
...
df_gapcheck <- df_series_orig %>%
  filter(!is.na(gap_class_lag) & gap_check == "1") %>%
  mutate(name_year = paste0(deployment, "_", Year)) %>%
  select(name_year, common_name, gap_class, gap_class_lag, diff_time) %>%
...
df_tbi_all <- read_csv("G:/Shared drives/ABMI Camera Mammals/data/processed/time-btwn-images/Table of time between photos within series by species incl 2019 May 2020.csv") %>%
  rename(common_name = Species, time_btwn_photos = TBP)
...
# 90% of cameras:
cam_90 <- floor(max(df_cam_op_2015$.) * 0.9)
...
data.frame(Date = days, Num_Cameras = df_cam_op_2015$.) %>%
  ...
  geom_vline(xintercept = as.numeric(summer_end15), 
             col = "darkred", size = 1, linetype = 2) +
  #geom_hline(yintercept = cam_90, col = "darkgreen", size = 1) +
  #annotate("text", x = as.POSIXct("2015-08-15 01:00:00"),
  #         y = (cam_90 + 20), label = "90% - March 19 to July 1", size = 3) +
  scale_x_datetime(date_breaks = "1 month", date_labels = "%b %Y") +
...
# 90% of cameras:
cam_90 <- floor(max(df_cam_op_2016$.) * 0.9)
...
data.frame(Date = days, Num_Cameras = df_cam_op_2016$.) %>%
  ...
  geom_vline(xintercept = as.numeric(summer_end16), 
             col = "darkred", size = 1, linetype = 2) +
  #geom_hline(yintercept = cam_90, col = "darkgreen", size = 1) +
  #annotate("text", x = as.POSIXct("2015-12-24 01:00:00"),
  #         y = (cam_90 + 20), label = "90% - March 20 to June 27", size = 3) +
  scale_x_datetime(date_breaks = "2 months", date_labels = "%b %Y") +
  scale_y_continuous(breaks = seq(0, 800, 100), limits = c(0, 800)) +
  annotate("text", 
           x = as.POSIXct(inter1_16@start + as.duration(inter1_16) / 2),
...
# 90% of cameras:
cam_90 <- floor(max(df_cam_op_2017$.) * 0.9)
...
data.frame(Date = days, Num_Cameras = df_cam_op_2017$.) %>%
  ...
  geom_vline(xintercept = as.numeric(summer_end17), 
             col = "darkred", size = 1, linetype = 2) +
  #geom_hline(yintercept = cam_90, col = "darkgreen", size = 1) +
  #annotate("text", x = as.POSIXct("2016-12-24 01:00:00"),
  #         y = (cam_90 + 20), label = "90% - March 8 to June 26", size = 3) +
  scale_x_datetime(date_breaks = "2 months", date_labels = "%b %Y") +
  scale_y_continuous(breaks = seq(0, 800, 100), limits = c(0, 800)) +
  annotate("text", 
           x = as.POSIXct(inter1_17@start + as.duration(inter1_17) / 2),
...
# 90% of cameras:
cam_90 <- floor(max(df_cam_op_2018$.) * 0.9)
...
data.frame(Date = days, Num_Cameras = df_cam_op_2018$.) %>%
  ...
  geom_vline(xintercept = as.numeric(summer_end18), 
             col = "darkred", size = 1, linetype = 2) +
  #geom_hline(yintercept = cam_90, col = "darkgreen", size = 1) +
  #annotate("text", x = as.POSIXct("2017-12-24 01:00:00"),
  #        y = (cam_90 + 20), label = "90% - Feb 26 to June 14", size = 3) +
  scale_x_datetime(date_breaks = "2 months", date_labels = "%b %Y") +
  scale_y_continuous(breaks = seq(0, 800, 100), limits = c(0, 800)) +
  annotate("text", 
           x = as.POSIXct(inter1_18@start + as.duration(inter1_18) / 2),
...
plot <- df_edd_final %>%
  ...
  labs(x = "Landcover Category",
       y = "",
       title = "Estimated effective detection distance (m) by species group.") +
...
<<<<<<< HEAD
load("G:/Shared drives/ABMI Camera Mammals/data/processed/detection-distance/predictions/Detection distances by site species and season.rData")
=======
...
>>>>>>> be4a0397e78396693d8e09844d2b5ee98b44aef5
